...
sub Mail::Sender::easy {
    ...
    my(@siteheaders);
    if (!defined($Mail::Sender::NO_X_MAILER)) {
	@siteheaders = (
    ...
    eval {
        local $Mail::Sender::SITE_HEADERS;

	if (!defined($Mail::Sender::NO_X_MAILER)) {
	    $Mail::Sender::SITE_HEADERS = join("\015\012", @siteheaders) || '';
	}
...
#               'msg'   => $text,
		...
		for my $attach (@$attachments) {
		    my($desc);
		    if (!exists($attach->{description})) {
			Carp::carp("Attachment must have a description\n");
		    }
		    $desc = $attach->{description};
		    if (!exists($attach->{ctype})) {
			$attach->{ctype} = 'text/plain';
		    }
		    if (!exists($attach->{_disptype})) {
			$attach->{_disptype} = $desc;
		    }
		    my $disp = qq(attachment; filename="$desc"; )
		      . qq(type=$attach->{_disptype});
		    if(defined $attach->{_inline} && $html) {
			$disp = qq(inline; filename="$desc";\r\nContent-ID: )
			  . qq(<$attach->{_inline}>);
		    }
		...
		Carp::carp("The attachment is not an array or hash ref\n");
	    }
        }
        $sndr->Close();
    };
